...
//   console.log("d", getUsers?.data?.meta);
...
//           current: page,
//           pageSize: 10,
//           total: getUsers?.data?.meta?.total, // Total count of inquiries from API response
...
function User() {
  ...
  const { data: getUsers, isLoading } = useGetUsersQuery(currentPage); // Use currentPage in query
  const [updateUser] = useUpdateUsersMutation();
  const [localUsers, setLocalUsers] = useState([]);
